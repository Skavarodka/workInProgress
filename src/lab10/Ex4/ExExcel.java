...
//Затем мы определяем основной метод программы main, который выбрасывает исключение IOException, связанное с записью файла.
//Далее мы создаем новую книгу Excel, используя класс XSSFWorkbook. Затем мы создаем новый лист в книге с названием "Товары",
//используя метод createSheet. После этого мы заполняем ячейки листа данными. Сначала мы создаем заголовок таблицы,
//используя метод createRow. Затем мы создаем строки данных и заполняем их значениями, используя метод createRow.
//В этом примере мы создали две строки данных, содержащие информацию о книге и компьютере соответственно.
//Каждая строка содержит три ячейки: название товара, характеристики и стоимость.
